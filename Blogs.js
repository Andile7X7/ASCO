const supabaseUrl = 'https://sfdozziezseywauuqddk.supabase.co';
    ...
    // Check admin role with fallback: first try localStorage, then check Supabase session
    async function getEffectiveRole() {
        ...
        // Fallback: check Supabase session to determine if this user is an admin
        try {
            ...
            // Cache the role in localStorage for future calls
    ...
    (async function init() {
        ...
        if (role !== 'admin') {
            // Non-admin: show only published blogs
            async function fetchBlogs() {
                try {
                    ...
                    console.log('Blogs (non-admin view):', blogs);
                } catch (err) {
            ...
            // Admin: show ALL blogs with Live/Unpublished status tags
            async function fetchAdminBlogs() {
                try {
                    ...
                    console.log('Blogs (admin view):', blogs);
                } catch (err) {
                    console.error('Unexpected error:', err.message);
                }
            }
